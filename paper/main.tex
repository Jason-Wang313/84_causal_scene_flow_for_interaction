...
\usepackage{url}
\usepackage{booktabs}
\title{Causal Scene Flow for Interaction: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about 3D perception for manipulation. The original research bet was: Infer which scene-flow components are caused by robot action versus passive motion. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item Aerial Manipulation with Contact-Aware Onboard Perception and Hybrid Control (2026)
\item HRIBench: Benchmarking Vision-Language Models for Real-Time Human Perception in Human-Robot Interaction (2025)
\item Natural teaching for humanoid robot via human-in-the-loop scene-motion cross-modal perception (2019)
\item Sensing the Action: Rethinking Sensor Modalities and Multi-Modal Fusion in Vision-Language-Action Models for Robotic Manipulation (2026)
\item DUEL: Depth visUal Ego-motion Learning for autonomous robot obstacle avoidance (2024)
\end{itemize}
